...
// var colorBrush = function(){
	...
	for (var c = 0; c < brushColorsArr.length; c++){
		brushColorsArr[c].addEventListener("click", function(){
			...
			var paintMe = document.getElementsByClassName("tinydiv");
			for (var j = 0; j < paintMe.length; j++){
				paintMe[j].addEventListener("click", function(){
					this.style.background = brushColor;
				});		
			}
			//	paintMe[j].addEventListener("dragover", function(){
			//		this.style.background = brushColor;
			//	})
